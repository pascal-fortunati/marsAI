...
import { isCatalogueAvailable, isPalmaresAvailable, isSubmissionOpen } from '../config/festival'
...
function FlagFR() {
  return (
    <svg viewBox="0 0 24 24" className="h-7 w-7 rounded-full">
      ...
      <circle cx="12" cy="12" r="11.5" fill="none" stroke="rgba(255,255,255,0.35)" />
    </svg>
  )
}
      ...
      {[
        ...
        return (
          <button
            ...
            style={{
              borderColor: active ? 'rgba(125,113,251,0.55)' : 'rgba(255,255,255,0.15)',
              background: active ? 'linear-gradient(135deg, rgba(125,113,251,0.22), rgba(255,92,53,0.2))' : 'rgba(255,255,255,0.04)',
              boxShadow: active ? '0 0 14px rgba(125,113,251,0.25)' : 'none',
            }}
          >
            <span className="flex items-center justify-center">{item.key === 'fr' ? <FlagFR /> : <FlagEN />}</span>
...
export function NavBar() {
  ...
  const currentLang: 'fr' | 'en' = i18n.language?.startsWith('fr') ? 'fr' : 'en'
  ...
  useEffect(() => {
    ...
    fetchPublicFestivalSettings(ac.signal).then(setSettings).catch(() => { })
        ...
        <div className="mx-auto max-w-screen-2xl px-8">
          <div className="flex items-center justify-between py-5">
            <div className="flex items-center gap-4">
              <Link to="/" className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-2xl">
              ...
              <div>
                <div className="f-orb text-base font-black text-white">
                  ...
                  <span className="ml-2 text-white/55">{t('nav.dashboard')}</span>
                </div>
                <div className="f-mono text-xs text-white/28">{stats.loading ? t('common.loading') : subtitle}</div>
              </div>
            </div>

            <div className="hidden sm:flex items-center divide-x divide-white/8">
              {[
                { label: t('nav.stats.films'), val: stats.filmsTotal, color: 'text-white' },
                { label: t('nav.stats.selected'), val: stats.selectedText, color: 'text-[#7d71fb]' },
                { label: t('nav.stats.pending'), val: stats.pendingCount, color: 'text-[#f59e0b]' },
                { label: t('nav.stats.validated'), val: stats.validatedCount, color: 'text-[#22c55e]', hideSm: true },
                { label: t('nav.stats.review'), val: stats.reviewCount, color: 'text-[#a855f7]', hideSm: true },
                { label: t('nav.stats.refused'), val: stats.refusedCount, color: 'text-[#ef4444]', hideSm: true },
              ].map((s) => (
                <div key={s.label} className={`${s.hideSm ? 'hidden lg:block' : ''} px-8 text-center`}>
                  <div className={`f-orb text-2xl font-black ${s.color}`}>{s.val}</div>
                  <div className="f-mono text-xs text-white/28 mt-0.5">{s.label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-0 border-t border-white/6">
              {tabs.map((t) => (
                <button
                  ...
                  style={{
                    color: activeTab === t.key ? marsaiColors.primary : 'rgba(255,255,255,.32)',
                    borderBottom: activeTab === t.key ? `2px solid ${marsaiColors.primary}` : '2px solid transparent',
                  }}
                >
                  {t.label}
      ...
      <div className="sticky top-0 z-50 border-b border-[#7d71fb]/20 bg-[#05030d]/90 backdrop-blur-xl shadow-[0_4px_60px_rgba(125,113,251,0.07)]">
        ...
        <div className="mx-auto max-w-screen-2xl px-8">
          <div className="flex items-center justify-between py-5">
            ...
            <div className="hidden sm:flex items-center divide-x divide-white/8">
              {[
                { label: t('nav.stats.films'), val: stats.total, color: 'text-white' },
                { label: t('jury.voted'), val: stats.voted, color: 'text-[#22c55e]' },
                { label: t('jury.remaining'), val: Math.max(0, stats.total - stats.voted), color: 'text-[#f59e0b]' },
                { label: t('nav.stats.progress'), val: `${stats.pct}%`, color: 'text-[#7d71fb]' },
              ].map((s) => (
    ...
    <>
      <div
        className={[
          ...
          scrolled
            ? 'border-b border-[#7d71fb]/20 bg-[#05030d]/90 backdrop-blur-xl shadow-[0_4px_60px_rgba(125,113,251,0.07)]'
            : 'bg-transparent',
        ].join(' ')}
      >
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#7d71fb]/70 to-transparent" />
        ...
        <nav className="mx-auto flex max-w-screen-2xl items-center gap-6 px-8 py-5">
          <Link to="/" className="group flex items-center gap-3 shrink-0">
            <div className="relative flex h-14 w-14 items-center justify-center overflow-hidden rounded-xl">
              <img src={marsAiLogo} alt="marsAI" className="h-full w-full object-contain" />
            </div>
            <div>
              <div className="f-orb text-sm font-black tracking-[0.15em] text-white">
                MARS<span className="text-[#ff5c35]">AI</span>
              </div>
              <div className="f-mono text-[8px] tracking-[0.2em] text-white/25">
                {t('nav.festival')}
                {tick % 2 === 0 ? '▮' : ' '}
              </div>
            </div>
          ...
          <div className="hidden flex-1 items-center gap-1 md:flex">
            ...
            <span className="f-mono text-[10px] text-white/15 tracking-widest">{t('nav.statusOpen')}</span>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <LangSwitch currentLang={currentLang} onChange={setLanguage} />
            <button className="flex flex-col gap-1 p-2 md:hidden" onClick={() => setMenuOpen(!menuOpen)}>
              <span
                className={`block h-px w-5 bg-white transition-all duration-300 ${menuOpen ? 'translate-y-1.5 rotate-45' : ''}`}
              />
        ...
        <div className={`overflow-hidden transition-all duration-300 md:hidden ${menuOpen ? 'max-h-64' : 'max-h-0'}`}>
          <div className="f-mono flex flex-col gap-px border-t border-white/8 bg-[#05030d]/95 px-4 py-3 backdrop-blur-xl">
            {[
              { to: '/submit', label: `// ${t('nav.submit').toUpperCase()}`, show: showSubmit },
              { to: '/catalogue', label: `// ${t('nav.catalogue').toUpperCase()}`, show: showCatalogue },
              { to: '/palmares', label: `// ${t('nav.palmares').toUpperCase()}`, show: showPalmares },
              { to: '/jury', label: `// ${t('nav.jury').toUpperCase()}`, show: showJury },
              { to: '/panel', label: `// ${t('nav.panel').toUpperCase()}`, show: showPanel },
            ]
              .filter((l) => l.show)
              .map(({ to, label }) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-2 rounded px-3 py-2.5 text-xs tracking-widest transition-all ${isActive ? 'bg-[#7d71fb]/10 text-[#7d71fb]' : 'text-white/40 hover:bg-white/5 hover:text-white/70'
                    }`
                  }
                >
                  {label}
