import { useTranslation } from "react-i18next";
import { Search } from "lucide-react";

type FilmSearchProps = {
  query: string;
  onSearch: (query: string) => void;
  disabled?: boolean;
};

export function FilmSearch({ query, onSearch, disabled }: FilmSearchProps) {
  const { t } = useTranslation();

  // TODO: appeler l'API /api/films?search=...
  return (
    <div className="rounded-[20px] border border-slate-800 bg-slate-900/45 p-4 text-white shadow-lg md:p-5">
      <div className="relative flex gap-2">
        <Search
          className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400"
          aria-hidden="true"
        />
        <input
          className="f-mono flex-1 rounded border border-slate-800 bg-slate-900 py-2 pl-12 pr-3 text-white placeholder:text-slate-400 hover:border-slate-700 focus:outline-none focus:ring-2 focus:ring-primary"
          placeholder={t("jury.searchPlaceholder")}
          value={query}
          onChange={(event) => onSearch(event.target.value)}
          disabled={disabled}
        />
      </div>
    </div>
  );
}
