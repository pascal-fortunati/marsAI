...
export function HomeView({
  ...
  const previewMode =
    preview?.enabled ?? previewParams.get("previewHome") === "1";
  const previewLang = preview?.lang ?? previewParams.get("lang");
  const previewEmbedded = preview?.embedded ?? false;
  const previewStandalone = previewMode && !previewEmbedded;
  ...
  useEffect(() => {
    ...
    if (previewMode && previewEmbedded) {
      if (
        (previewLang === "fr" || previewLang === "en") &&
        i18n.language !== previewLang &&
        lastEmbeddedLang.current !== previewLang
      ) {
        lastEmbeddedLang.current = previewLang;
        void i18n.changeLanguage(previewLang);
      }
      const nextTranslations = embeddedSettings?.homeTranslations
        ? JSON.stringify(embeddedSettings.homeTranslations)
        : null;
    ...
    fetchPublicFestivalSettings(ac.signal)
      .then((s) => {
        ...
        if (previewMode) {
          ...
          if (previewLang === "fr" || previewLang === "en")
            void i18n.changeLanguage(previewLang);
        }
        setSettings(next);
  ...
  useEffect(() => {
    ...
    const send = () => {
      ...
      window.parent.postMessage(
        { type: "marsai-preview-height", height },
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (
        ...
        return;
      const rawTarget = payload.target.trim();
      const target =
        rawTarget === "home.eyebrowBadge"
          ? "home.eyebrow"
          : rawTarget === "home.eyebrowText"
            ? "home.eyebrowText"
            : rawTarget;
      setPreviewFocus(target);
      const selector = `[data-preview-target="${target.replace(/"/g, '\\"')}"]`;
  ...
  }, [previewMode, previewEmbedded]);

  const previewFocusTarget = previewFocus;
  const isPreviewFocused = (target: string) =>
    previewFocusTarget === target ||
    (target === "home.eyebrow" &&
      (previewFocusTarget === "home.eyebrowBadge" ||
        previewFocusTarget === "home.eyebrowText")) ||
  ...
  const statusLabels = useMemo(() => {
    ...
    const enDefaults = resources.en.translation.home as {
      live?: unknown;
      finished?: unknown;
    };
    const frDefaults = resources.fr.translation.home as {
      live?: unknown;
      finished?: unknown;
    };
    return {
      live:
        normalize(live) === normalize(String(enDefaults.live || ""))
          ? String(frDefaults.live || live)
          : live,
      finished:
  ...
  const phaseLabels = useMemo(() => {
    const normalize = (v: string) =>
      v.toLowerCase().trim().replace(/\s+/g, " ");
    const normalizePhaseText = (
      key: "p1Title" | "p1Desc" | "p2Title" | "p2Desc" | "p3Title" | "p3Desc",
    ) => {
      const value = t(`home.phases.${key}`);
      if (!isFrLanguage) return value;
      const enDefault = (
        resources.en.translation.home.phases as Record<string, string>
      )[key];
      const frDefault = (
        resources.fr.translation.home.phases as Record<string, string>
      )[key];
      ...
      const isEnglishFallback =
        ...
        (key === "p2Desc" &&
          normalizedValue.includes("selected by the jury")) ||
        (key === "p3Desc" &&
          (normalizedValue.includes("jury prize") ||
            normalizedValue.includes("public prize")));
      return isEnglishFallback ? frDefault : value;
    };
    return [
      {
        title: normalizePhaseText("p1Title"),
        desc: normalizePhaseText("p1Desc"),
      },
      {
        title: normalizePhaseText("p2Title"),
        desc: normalizePhaseText("p2Desc"),
      },
      {
        title: normalizePhaseText("p3Title"),
        desc: normalizePhaseText("p3Desc"),
      },
    ];
  }, [isFrLanguage, t]);

  const { phases, currentIndex } = useMemo(
    () => computePhases(shownSettings, nowMs, phaseLabels),
    [shownSettings, nowMs, phaseLabels],
  );
  ...
  const targetIso =
    currentIndex === 0
      ? shownSettings?.phase1CloseIso
      : currentIndex === 1
        ? shownSettings?.phase2CatalogueIso
        : shownSettings?.phase3PalmaresIso;
  ...
  const statTo = (
    key: "countries" | "filmsExpected" | "visitors",
    fallback: number,
  ) => {
    const raw = String(
      t(`home.statsNumbers.${key}`, { defaultValue: String(fallback) }) ??
        fallback,
    );
  ...
  const stats = [
    {
      to: statTo("countries", 120),
      suffix: "+",
      label: t("home.stats.countries"),
      col: marsaiColors.primary,
    },
    {
      to: statTo("filmsExpected", 600),
      suffix: "+",
      label: t("home.stats.filmsExpected"),
      col: marsaiColors.accent,
    },
    {
      to: statTo("visitors", 3000),
      suffix: "",
      label: t("home.stats.visitors"),
      col: marsaiColors.primary2,
    },
  ];
  ...
  return (
    <div
      ...
      style={previewStandalone ? undefined : { minHeight: "100vh" }}
    >
      <section
        className={[
          previewStandalone
          ...
          highlightClass("heroImageUrl", "rounded-2xl"),
        ].join(" ")}
        data-preview-target="heroImageUrl"
        style={
      ...
      >
        ...
        >
          <div
            ...
            style={{ borderColor: withAlpha(marsaiColors.primary, 0.06) }}
          />
          <div
            className="absolute inset-[60px] rounded-full border"
            style={{ borderColor: withAlpha(marsaiColors.accent, 0.04) }}
          />
        </div>
        ...
        <div
          className={[
            ...
            previewStandalone
              ? "justify-start pt-10 pb-10"
              : "flex-1 justify-center py-24",
          ].join(" ")}
        >
          <div
            className={[
              "mb-8 flex items-center gap-2.5 rounded-full border border-[#7d71fb]/30 bg-[#7d71fb]/8 px-5 py-2 backdrop-blur-sm",
              highlightClass("home.eyebrow", "rounded-full"),
              isPreviewFocused("home.eyebrow") ? "bg-[#7d71fb]/20" : "",
            ].join(" ")}
            data-preview-target="home.eyebrow"
            style={{
              animation: "fadeUp 0.5s ease-out 0.1s both",
              boxShadow: "0 0 30px rgba(125,113,251,0.15)",
            }}
          >
            ...
            <span
              className={[
                "f-mono text-[11px] uppercase tracking-[0.2em] text-[#7d71fb] rounded-sm px-1",
                highlightClass("home.eyebrowText", "rounded-sm"),
              ].join(" ")}
              data-preview-target="home.eyebrowText"
            >
              {submissionsOpen ? t("home.eyebrow") : t("home.eyebrowClosed")}{" "}
              {t("home.eyebrowSuffix")}
            </span>
          </div>

          <div
            className={[
              "f-mono mb-4 text-xs tracking-[0.4em] text-white/25 uppercase",
              highlightClass("home.terminal"),
            ].join(" ")}
          ...
          >
            <div
              className={["text-white", highlightClass("home.title1")].join(
                " ",
              )}
              data-preview-target="home.title1"
            >
              {t("home.title1")}
            </div>
            <div
              className={["text-white/30", highlightClass("home.title2")].join(
                " ",
              )}
              data-preview-target="home.title2"
            >
              {t("home.title2")}
            </div>
            <div
              data-preview-target="home.title3"
              className={highlightClass("home.title3")}
              style={{
                backgroundImage:
                  "linear-gradient(90deg, #7d71fb, #a970fc, #ff5c35, #7d71fb)",
          ...
          <p
            className={[
              "f-mono mt-6 max-w-xl text-sm leading-relaxed text-white/40",
              highlightClass("home.heroText"),
            ].join(" ")}
            data-preview-target="home.heroText"
          ...
          {shownSettings?.festivalDescription ? (
            <p
              className="f-mono mt-3 max-w-xl text-xs leading-relaxed text-white/35"
              style={{ animation: "fadeUp 0.7s ease-out 0.58s both" }}
            >
              {shownSettings.festivalDescription}
            </p>
          ) : null}
          ...
          <div
            className={[
              ...
              highlightClass("home.ctaZone"),
            ].join(" ")}
            data-preview-target="home.ctaZone"
          ...
          >
            ...
            {!submissionsOpen && !phase2Started ? (
              <p className="f-orb text-sm font-bold text-white/75">
                {t("home.phaseNoticePhase2")}
              </p>
            ) : null}
            {!submissionsOpen && phase2Started && !phase3Finished ? (
              <>
                <p className="f-orb text-sm font-bold text-white/75">
                  {t("home.phaseNoticePhase3")}
                </p>
                <Button
                  asChild
                  className="f-orb group relative overflow-hidden rounded-full px-8 text-xs font-bold uppercase tracking-widest text-white transition-all duration-300"
                  ...
                  <Link to="/catalogue" className="flex items-center gap-3">
                    ...
                    >
                      ...
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="m8.25 9.75 3.75 2.25-3.75 2.25v-4.5Z"
                      />
                    </svg>
                  ...
                  <Link to="/palmares" className="flex items-center gap-3">
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M8.25 6.75h7.5v1.5a3.75 3.75 0 0 1-3 3.674V14.25h2.25a.75.75 0 0 1 0 1.5h-6a.75.75 0 0 1 0-1.5h2.25v-2.326a3.75 3.75 0 0 1-3-3.674v-1.5Z"
                      />
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M8.25 8.25H6a2.25 2.25 0 0 0 2.25 2.25M15.75 8.25H18a2.25 2.25 0 0 1-2.25 2.25"
                      />
                    </svg>
          ...
          >
            <div className="mb-8 flex items-center gap-4">
              ...
              <span
                className={[
                  "f-mono text-[9px] uppercase tracking-[0.3em] text-white/20",
                  highlightClass("home.timeline"),
                ].join(" ")}
                data-preview-target="home.timeline"
              >
                {t("home.timeline")}
              </span>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              >
                ...
                <div className="mb-4 flex items-center justify-between gap-3">
                  ...
                  <div className="f-orb text-sm font-bold text-white/80 text-right leading-none">
                    {phaseTitle}
                  </div>
                </div>
                {targetIso ? (
                  ...
                  <p className="f-mono text-sm uppercase tracking-widest text-white/30">
                    {t("home.dateTbd")}
                  </p>
                )}
              </div>
                ...
                {phases.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      <div className="f-orb text-[11px] font-bold text-white/80 leading-tight">
                        {p.title}
                      </div>
                      <div className="f-mono text-[9px] text-white/30 mt-0.5">
                        {p.desc}
                      </div>
                    </div>
                    {p.active ? (
                      <span className="f-mono shrink-0 rounded-full bg-[#7d71fb]/15 px-2 py-0.5 text-[8px] uppercase tracking-widest text-[#7d71fb]">
                        {phase3Finished && p.num === "03"
            ...
            {stats.map((s, idx) => (
              <div
                ...
                className={[
                  ...
                  highlightClass(
                    idx === 0
                      ? "home.statsNumbers.countries"
                      : idx === 1
                        ? "home.statsNumbers.filmsExpected"
                        : "home.statsNumbers.visitors",
                  ),
                ].join(" ")}
                data-preview-target={
                  idx === 0
              ...
              >
                <span
                  className="f-orb text-4xl font-black"
                  style={{ color: s.col }}
                >
                  <AnimCounter
                    to={s.to}
                ...
                <span className="f-mono text-[9px] uppercase tracking-[0.25em] text-white/30">
                  {s.label}
                </span>
              </div>
            ))}
        ...
        {shownSettings?.footerText ? (
          <div
            className={[
              "relative z-10 pb-6 text-center",
              highlightClass("footerText"),
            ].join(" ")}
            data-preview-target="footerText"
          >
            <span className="f-mono text-[10px] uppercase tracking-[0.2em] text-white/30">
              {shownSettings.footerText}
            </span>
          </div>
        ) : null}
      ...
      <section
        className={
          previewStandalone
            ? "relative mx-auto max-w-6xl px-5 pt-4 pb-6"
            : "relative mx-auto max-w-6xl px-5 py-20"
        }
      >
        ...
        <div className="relative grid gap-5 lg:grid-cols-12">
          <div
            className={[
              "relative overflow-hidden rounded-3xl p-8 lg:col-span-8 md:p-10",
              highlightClass("home.featureSection", "rounded-3xl"),
            ].join(" ")}
          ...
          >
            ...
            <div
              className={[
                "mb-5 inline-flex items-center gap-2 rounded border border-white/10 bg-white/4 px-3 py-1",
                highlightClass("home.place"),
              ].join(" ")}
              data-preview-target="home.place"
            >
              <svg
                className="h-3 w-3 text-[#ff5c35]"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
              ...
              <span className="f-mono text-[10px] tracking-widest text-white/40">
                {t("home.place")}
              </span>
            </div>
            ...
            <h2
              className={[
                "f-orb text-2xl font-black text-white md:text-3xl",
                highlightClass("home.featureTitle1"),
              ].join(" ")}
              data-preview-target="home.featureTitle1"
            >
              {t("home.featureTitle1")}
              <br />
              <span
                className={highlightClass("home.featureTitle2")}
                data-preview-target="home.featureTitle2"
                style={{
                  backgroundImage: "linear-gradient(90deg, #7d71fb, #ff5c35)",
                  WebkitBackgroundClip: "text",
                  WebkitTextFillColor: "transparent",
                }}
              >
            ...
            <p
              className={[
                "f-mono mt-4 max-w-lg text-sm leading-relaxed text-white/40",
                highlightClass("home.featureText"),
              ].join(" ")}
              data-preview-target="home.featureText"
            >
              {t("home.featureText")}
            </p>

            <div className="mt-5 flex flex-wrap gap-2">
              {(t("home.featureTags", { returnObjects: true }) as string[]).map(
                (tag) => (
                  <span
                    key={tag}
          ...
          <div className="flex flex-col gap-3 lg:col-span-4">
            {stats.map((s) => (
              ...
              >
                <div
                  ...
                  style={{
                    background: `linear-gradient(90deg, transparent, ${s.col}70, transparent)`,
                  }}
                />
                <div
                  className="f-orb text-4xl font-black"
                ...
                <div className="f-mono mt-1 text-[9px] uppercase tracking-[0.2em] text-white/30">
                  {s.label}
                </div>
              </div>
            ))}
            ...
            >
              ...
              <div
                className={[
                  "f-mono text-[8px] uppercase tracking-[0.25em] text-[#ff5c35]/50 mb-1",
                  highlightClass("home.themeTitle"),
                ].join(" ")}
                data-preview-target="home.themeTitle"
              >
                {t("home.themeTitle")}
              </div>
              <div
                className={[
                  "f-orb text-sm font-bold text-white/60",
                  highlightClass("home.themeQuote"),
                ].join(" ")}
                data-preview-target="home.themeQuote"
              >
                {t("home.themeQuote")}
              </div>
            </div>
